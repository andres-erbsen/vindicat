...
main = do
  ...
  let broadcast pkt = readMVar srifsV >>= mapM_ (flip sendPacketSRBS pkt)
...
receive :: KeyPair -> MVar Atlas -> (ByteString -> IO ()) -> CallbackSRBS
  ...
  receivep iface _ rawp = case decode rawp of
    Right etherp -> case decode (etherData etherp) of
      Right packet -> case packet of
        DevicePack dev -> modifyMVar atlasV remember `andif` broadcast etherp
          where remember atl = return $ insertDevice dev atl
        LinkPack  link -> modifyMVar atlasV remember `andif` broadcast etherp
          where remember atl = return $ insertLink link atl
        LinkReqPack lh -> do
          atl <- readMVar atlasV
          let other = getDevice atl (linkHalfLeftEnd lh)
          when (shouldAcceptLink atl lh) $ sendTo other $ acceptLink kp lh
